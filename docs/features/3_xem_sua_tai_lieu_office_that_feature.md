...
- **Thư viện/asset mới cần thêm:**
  ...
  - `x2t` build WebAssembly (bộ chuyển đổi định dạng, ~9.2 MB/bản trong repo tham khảo) — đúng bộ dùng để khứ hồi OOXML. **Không lấy được từ image `onlyoffice/documentserver`**: `x2t` trong image chính thức là binary ELF native (~45 MB), không phải WASM — bản WASM là build Emscripten riêng, cần tự build hoặc vendor từ nguồn khác (xác minh xuất xứ trước khi dùng)
...
- **Ảnh hưởng kích thước bundle:** lớn, **chưa đo được chính xác** — `x2t.wasm` xác nhận ~9.2 MB; phần `sdkjs`/`web-apps`/fonts không nằm trong Git của repo tham khảo (lấy từ Docker image lúc build) nên chưa đo trực tiếp được, cần spike đo trước khi cam kết task chi tiết (xem mục 8)

## 7. User story

- [ ] [US-8: Đo kích thước thật + dựng pipeline vendor asset ONLYOFFICE (spike)](../stories/8_do_kich_thuoc_pipeline_asset_story.md)

Story tiếp theo (tích hợp editor xem, sửa + lưu khứ hồi, UI "Tải mã nguồn" AGPL) chỉ viết **sau khi** US-8 trả lời được rủi ro ở mục 8 — số lượng và tên các US sau đó tuỳ vào kết quả spike, không cam kết trước.
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dung lượng server-side đã đo (~1.4 GB: `sdkjs` 418 MB + `web-apps` 783 MB + `sdkjs-plugins` 37 MB + `fonts` 162 MB, xem [TASK-18](../tasks/18_do_kich_thuoc_asset_onlyoffice_task.md)), nhưng **dung lượng trình duyệt thật sự tải lần đầu vẫn chưa đo được** | Số trên đĩa là cận trên (mọi editor/ngôn ngữ), không phải payload thật; chưa biết có giữ được lời hứa "tải lần đầu vài giây" (`CLAUDE.md`) hay không   | Cần dựng lại kỹ thuật chặn `XMLHttpRequest`/`fetch` của `office-website` để có editor chạy đúng kiến trúc mục tiêu rồi đo qua `browser_network_requests` — việc này lớn hơn phạm vi một spike, chưa lên kế hoạch |
| `x2t` bản WASM không lấy được trực tiếp từ image `onlyoffice/documentserver` (image chỉ có bản ELF native) — cần tự build Emscripten hoặc vendor nguồn khác                                                                                                   | Build pipeline phức tạp hơn "chỉ COPY từ Docker image" như dự tính ban đầu; vendor bản build sẵn của bên thứ ba kéo theo câu hỏi xuất xứ/tính toàn vẹn | Cần quyết định: tự build `x2t` sang WASM, hay vendor bản của `office-website`/nguồn khác kèm xác minh — ai chịu trách nhiệm build/verify chưa rõ                                                                 |
...
| ONLYOFFICE bản gốc có thể gọi ra ngoài (dictionary, help, translate CDN) mặc định                                                                                                                                                                             | Vi phạm bất biến "riêng tư là mặc định" nếu không rà soát/tắt hết                                                                                      | Chưa rà được (phụ thuộc có editor chạy đúng kiến trúc, xem rủi ro dòng đầu) — bắt buộc kiểm chứng bằng `browser_network_requests` trước khi coi feature đạt yêu cầu                                              |
