...
- **Thư viện/asset mới cần thêm** (đã vendor xong ở [TASK-19](../tasks/19_vendor_pipeline_onlyoffice_task.md), tổng **267.1 MB** đã lọc theo nhu cầu US-9, xem bảng chi tiết trong task đó):
  ...
  - Fonts hệ thống ONLYOFFICE dùng để render đúng — 160.6 MB, chưa lọc (rủi ro tối ưu thêm, để lại làm task riêng sau nếu payload đo được ở TASK-23 không đạt kỳ vọng)
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Dung lượng server-side đã đo (~1.4 GB: `sdkjs` 418 MB + `web-apps` 783 MB + `sdkjs-plugins` 37 MB + `fonts` 162 MB, xem [TASK-18](../tasks/18_do_kich_thuoc_asset_onlyoffice_task.md)), nhưng **dung lượng trình duyệt thật sự tải lần đầu vẫn chưa đo được**                                                        | Số trên đĩa là cận trên (mọi editor/ngôn ngữ), không phải payload thật; chưa biết có giữ được lời hứa "tải lần đầu vài giây" (`CLAUDE.md`) hay không | Đo ở TASK-23 của [US-9](../stories/9_xem_sua_docx_onlyoffice_story.md), khi editor `.docx` chạy thật qua `browser_network_requests` |
...
| ~~ONLYOFFICE bản gốc có thể gọi ra ngoài (dictionary, help, translate CDN) mặc định~~ — **đã kiểm chứng ở mode Xem**: `browser_network_requests` khi mở `.docx` thật xác nhận 0 request ra ngoài origin của app (chỉ Google Fonts sẵn có của LocalOffice, không liên quan tài liệu). Còn luồng lưu (TASK-23) chưa đo | Vi phạm bất biến "riêng tư là mặc định" nếu không rà soát/tắt hết                                                                                    | TASK-21 đã đo phần Xem. Đo nốt phần Lưu ở TASK-23 của US-9 trước khi coi feature đạt yêu cầu                                        |
