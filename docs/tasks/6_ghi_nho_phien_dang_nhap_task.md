...
- [x] Kiểm thử qua Playwright MCP (`browser_run_code_unsafe` gọi `page.context().clearCookies()`, vì cookie phiên là httpOnly nên không xoá được qua `document.cookie`): xoá cookie `better-auth.session_token` → xác nhận qua `curl` sau đó (MCP rớt kết nối giữa chừng — xem ghi chú bên dưới): không cookie → về trạng thái chưa đăng nhập, không lỗi
...
**MCP rớt kết nối lần nữa khi test xoá cookie**: đúng bug console-piping đã ghi ở TASK-3/TASK-9 (log console phía server bị khuếch đại). Sau khi rớt, chuyển sang xác nhận qua `curl` trực tiếp tới server (không qua browser) cho cả hai trường hợp "không cookie" và "cookie rác" — kết quả nhất quán với kỳ vọng ban đầu (không crash, `get-session` trả `null`). Không lặp lại việc report bug này thêm ở đây, đã có ghi chú đầy đủ ở TASK-3.
